...
namespace View
{
    ...
    void render_wall_slice(Canvas* canvas, int col, Wall* wall, Camera* camera, float sector_height, float wall_slice_bottom, float wall_slice_top, float ray_intersection_x, float ray_intersection_y)//, float ray_intersection_distance)
    {
        float wall_slice_clipped_bottom = wall_slice_bottom > 0 ? wall_slice_bottom : 0;
        float wall_slice_clipped_top = wall_slice_top < canvas->height ? wall_slice_top : canvas->height;
        ...
        float texture_slice_y = wall_slice_bottom < wall_slice_clipped_bottom ? wall_slice_bottom * -texture_step_y : 0;
    ...
    void render_floor(Canvas* canvas, int col, Sector* sector, Camera* camera, float ray_dir_angle, float near_clip, float horizont_line, float portal_slice_bottom, float wall_slice_bottom)
    {
        ...
        float wall_slice_clipped_bottom = wall_slice_bottom < canvas->height ? wall_slice_bottom : canvas->height;
    ...
    void render_ceiling(Canvas* canvas, int col, Sector* sector, Camera* camera, float ray_dir_angle, float near_clip, float horizont_line, float portal_slice_top, float wall_slice_top)
    {
        ...
        float wall_slice_clipped_top = wall_slice_top > 0 ? wall_slice_top : 0;
    ...
    void render_column(Canvas* canvas, int col, World* world, Sector* sector, Camera* camera, float ray_dir_angle, float near_clip, float portal_slice_bottom, float portal_slice_top)
    {
        ...
        if (wall_index != -1) {
            ...
            if ((*(*sector).walls[wall_index]).portal_to != -1) {
                ...
                render_wall_slice(canvas, col, (*sector).walls[wall_index], camera, (*sector).top - (*sector).bottom, wall_slice_bottom, wall_slice_top, ray_intersection_x, ray_intersection_y);//, ray_intersection_distance);
            }
